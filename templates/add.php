<form class="form form--add-lot container<?php if (count($errors) > 0): ?> form--invalid<?php endif; ?>"
            ...
            <select id="category" name="category_id">
                ...
                <?php foreach ($categories as $value): ?>
                    <option value="<?= $value['id']; ?>"
                            <?php if (!empty($_POST['category_id'])): ?>selected<?php endif; ?>><?= $value['category_name']; ?></option>
                <?php endforeach; ?>
            </select>
    ...
    <div
        ...
        <textarea id="message" name="description" placeholder="Напишите описание лота"><?= getPostVal(
                'description'
            ); ?></textarea>
